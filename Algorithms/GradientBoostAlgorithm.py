...
def gradientBoost(data_train, data_test, target_train, target_test):
    '''
        This function train a model with GradientBoostingClassifier algorithm from sklearn and
    ...
    # make a GradientBoostingClassifier model to apply the Decision Tree alogorithm on data
    gradient_boost = GradientBoostingClassifier(n_estimators=100)

    # fit the model with training data
    gradient_boost.fit(data_train, target_train)

    # get predictions on test data
    prediction = gradient_boost.predict(data_test)
